        ...
        if not user_db.check_user(user_id):
            ...
            if mention is None or amount is None:
                ...
                embed.add_field(
                    ...
                    value=" :mag: ")
                embed.set_footer(
                    text="CPUchain tipbot {0} [Owner: {1}]".format(config.VERSION, self.bot.get_user(config.OWNER_ID)),
                    icon_url=self.bot.user.avatar_url_as(format='png', size=256))
                await ctx.channel.send(embed=embed)
                ...
                if amount < Decimal('0.00000001'):
                    ...
                    if len(tipto) != 18 and len(tipto) != 17: # length of discord user id is 18 or 17
                        ...
                        embed.add_field(
                            ...
                            value="Your balances ```{0} CPU```".format(client.getbalance(tipfrom, config.CONFIRM)))
                        embed.set_footer(text="CPUchain tipbot {0} [Owner: {1}]".format(config.VERSION, self.bot.get_user(config.OWNER_ID)),
                                         icon_url=self.bot.user.avatar_url_as(format='png', size=256))
                        ...
                        if tipto == str(self.bot.user.id):
                            try:
                                move_istrue = client.move(tipfrom, 'tipcpu_wallet', float(amount))
                            except:
                                embed = discord.Embed(color=0xff0000)
                                embed.set_author(
                                    name=ctx.author.display_name,
                            ...
                            if move_istrue:
                                ...
                                embed.set_author(
                                    ...
                                    icon_url=ctx.author.avatar_url_as(format='png', size=256))
                                embed.add_field(
                                    name="{0} tipped to {1} `{2} CPU`".format(ctx.author.display_name,
